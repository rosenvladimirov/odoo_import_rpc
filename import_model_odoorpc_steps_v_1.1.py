...
STEPS['step_8']['SEARCH_DOMAIN'] = [('attribute_line_ids', '=', False), ('categ_id', 'not in', [445])]
...
# [10] product.product
# ('product_tmpl_id', 'not in', [4034, 3913, 4127, 8247, 8251, 8255, 8244, 8246, 8243, 8248, 8259, 8261, 8260, 8487]),
STEPS['step_10']['MODEL'] = 'product.product'
STEPS['step_10']['TARGET_MODEL'] = ''
STEPS['step_10']['SEARCH_DOMAIN'] = [('product_tmpl_id', 'not in', [4034, 3913, 4127, 8247, 8251, 8255, 8244, 8246, 8243, 8248, 8259, 8261, 8260, 8487])]
...
STEPS['step_10']['FIELDS'] = ['name', 'product_tmpl_id', 'active', 'standard_price', 'volume', 'weight',
                              'default_code', 'tracking', 'attribute_value_ids']
...
STEPS['step_10']['RELATIONAL_FIELDS'] = ['attribute_value_ids', 'product_tmpl_id']
...
STEPS['step_28']['SEARCH_DOMAIN'] = [('id', 'not in', [373, 535])]
STEPS['step_28']['SEARCH_DOMAINS'] = [[('id', '!=', False)]]
STEPS['step_28']['FIELDS'] = ['product_id', 'product_qty', 'product_uom_id', 'sequence', 'bom_id',
                              'attribute_value_ids', 'sequence', 'routing_id']
STEPS['step_28']['CHILD_FIELDS'] = []
STEPS['step_28']['SKIP_FIELDS'] = []
STEPS['step_28']['RELATIONAL_FIELDS'] = ['bom_id', 'product_id', 'product_uom_id', 'routing_id', 'attribute_value_ids']
...
STEPS['step_29']['SKIP_FIELDS'] = ['location_id']
...
STEPS['step_30']['FIELDS'] = ['name', 'sequence', 'routing_id', 'note', 'time_mode_batch', 'time_cycle_manual',
                              'time_cycle', 'workorder_count', 'batch', 'batch_size', 'company_id', 'workcenter_id']
STEPS['step_30']['CHILD_FIELDS'] = []
STEPS['step_30']['SKIP_FIELDS'] = ['location_id']
STEPS['step_30']['RELATIONAL_FIELDS'] = ['routing_id', 'company_id', 'workcenter_id']
STEPS['step_30']['FIELD_MAPPING'] = {
}
STEPS['step_30']['ORDER'] = 'id'
STEPS['step_30']['COMPARE_FILED'] = ''
...
STEPS['step_31']['SKIP_FIELDS'] = ['location_id']
...
STEPS['step_32']['SKIP_FIELDS'] = ['location_id']
        ...
        if isinstance(field_value, (list, tuple)) \
            ...
            if field_name == 'parent_id':
                ...
                parent_id = odoo_dest.search_and_read(
                    'ir.model.data',
                    [
                        ('name', '=', f'source_stock_production_lot_{field_value[0]}'),
                        ('model', '=', 'stock.lot')],
                    ['res_id'])
                print('lot_id', parent_id, f'source_stock_production_lot_{field_value[0]}')
                if parent_id:
                    relation_record[field_name] = parent_id[0]['res_id']
                else:
                    relation_record['block'] = True
                ...
                parent_id = odoo_dest.search_and_read(
                    'ir.model.data',
                    [
                        ('name', '=', f'source_mrp_bom_{field_value[0]}'),
                        ('model', '=', 'mrp.bom')],
                    ['res_id'])
                print(parent_id, f'source_mrp_bom_{field_value[0]}')
                if parent_id:
                    relation_record[field_name] = parent_id[0]['res_id']
                # else:
                #     relation_record['block'] = True
                ...
                parent_id = odoo_dest.search_and_read(
                    ...
                    ['res_id'])
                print(parent_id, f'source_stock_location_{field_value[0]}')
                if parent_id:
                    relation_record[field_name] = parent_id[0]['res_id']
                else:
                    source_id = odoo_src.search_and_read(
                ...
                parent_id = odoo_dest.search_and_read(
                    'ir.model.data',
                    [
                        ('name', '=', f'source_mrp_routing_{field_value[0]}'),
                        ('model', '=', 'mrp.routing')],
                    ['res_id'])
                print(parent_id, f'source_mrp_routing_{field_value[0]}')
                if parent_id:
                    relation_record[field_name] = parent_id[0]['res_id']
                else:
                    relation_record['block'] = True
                ...
                parent_id = odoo_dest.search_and_read(
                    'ir.model.data',
                    [
                        ('name', '=', f'source_mrp_workcenter_{field_value[0]}'),
                        ('model', '=', 'mrp.workcenter')],
                    ['res_id'])
                print(parent_id, f'source_mrp_workcenter_{field_value[0]}')
                if parent_id:
                    relation_record[field_name] = parent_id[0]['res_id']
                else:
                    relation_record['block'] = True
            ...
            if field_name == 'attribute_line_ids':
            ...
            elif field_name == 'attribute_value_ids' and relation_model == 'mrp.bom.line':
                for id_res in field_value:
                    parent_id = odoo_dest.search_and_read(
                        'ir.model.data',
                        [
                ...
                parent_id = odoo_dest.search_and_read(
                    ...
                    [
                        ...
                        ('model', '=', 'product.product')],
                    ['res_id'])
                print('product_id', parent_id, f'source_product_product_{field_value[0]}')
                if parent_id:
                    relation_record[field_name] = parent_id[0]['res_id']
                else:
                    relation_record['block'] = True
    ...
    for search_domain in SEARCH_DOMAINS:
        while start:
            ...
            for record in source_data:
                ...
                print(f'Version: {odoo_dest.odoo.version.split(".")}')
                ...
                if not existing_id and COMPARE_FILED and record.get(COMPARE_FILED):
                    print('Skip record', record[COMPARE_FILED].strip())
                    ...
                    if target_id:
                        ...
                        # odoo_dest.odoo.env['ir.model.data'].create({
                        #     'name': f'source_{MODEL.replace(".", "_")}_{processed_record["id"]}',
                        #     'model': target_model,
                        #     'module': 'remote_multi_company',
                        #     'res_id': target_id[0]['id'],
                        # })
                ...
                print('Record to save #:',
                      {k: v for k, v in processed_record.items() if k not in ['datas', 'image', 'image_1920']},
                      target_model)
